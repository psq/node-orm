...
ORM.prototype.setLogQueries = function(log) {
  this._db.setLogQueries(log);
}
...
ORM.prototype.define = function (model, fields, colParams) {
  var orm = this,
      ...
      meta = {model: model, fields: fields, colParams: colParams};
      // add_id = (!fields._params || fields._params.indexOf("no-id") == -1);

  // console.log("idProperty: ", idProperty);

  var Model = function (data, opts, newRecord) {
    events.EventEmitter.call(this);

    this._dataPending = 0;
    this._dataHash = null;
    this._newRecord = (newRecord === undefined) ? true : newRecord;
    this._meta = meta;
    this._model = orm._models[model];
    // this._primary = null;
    ...
    if (colParams && colParams.methods) {
      for (var k in colParams.methods) {
  ...
  Model._getMeta = function() {
    return meta;
  };

  Model._getName = function() {
    return meta.model;
  }
    ...
    for (var k in fields) {
      ...
      switch (fields[k].type) {
        ...
        case "object":
          ...
          break;
        case "data":
          data[k] = encoders.encodeByteA(this[k]);
          break;
        default:
  ...
  Model.prototype.save = function () {
    ...
    var saveRecord = function () {
      ...
      // console.log("save self:", util.inspect(self));
      
      orm._db.saveRecord(idProperty, model, data, self._meta, self._newRecord, function (err, id) {
        if (err) {
    ...
    // give a chance to beforeSave hook to modify data, but will this get validated
      ...
      orm._db.clearRecords(model, {
        ...
        "callback": function (err, info) {
          /*
            ...
            TODO: is this a good idea to allow to re-save...  need soft delete anyway.
          */
  ...
  Model.sync = function (callback) {
    orm._db.createCollection(model, fields, associations, callback);
  };
  ...
  Model.get = function (id, callback) {
    ...
    orm._db.selectRecords(model, {
      ...
      "callback"  : function (err, data) {
        ...
        (new Model(data[0], modelOpts, false)).ready(callback);
      }
    });
  };
    ...
    if (callback !== null) {
      config.callback = function (err, data) {
        ...
        for (var i = 0; i < data.length; i++) {
          data[i] = new Model(data[i], modelOpts, false);
      ...
      (function (m) {
        query.on("record", function (record) {
          (new Model(record, modelOpts, false)).ready(function (m) {
            m.emit("record", m);
          });
        });
